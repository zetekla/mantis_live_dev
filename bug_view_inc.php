...
# along with MantisBT.  If not, see <http://www.gnu.org/licenses/>.
	...
	// bug api for processing email reported bugs with inline html image.
...
#	echo '<tr class="spacer"><td colspan="6"></td></tr>';
	...
	if ( $tpl_show_description ) {
		...
		echo '<td colspan="5">'; // comment out this section for addtion of Email reporting , $tpl_description, '</td>';
		// generate array of attachments to bind with email scr image code for display as inline html or return normal description if bug was not generated through email reporting.
		$t_list_attachments = bug_inline_get_attachments($f_bug_id);
		if (ERP_bug_from_email( $f_bug_id )){
			// require_css('similar_styling.css');
			$tpl_description = show_description($f_bug_id, $t_list_attachments)->description;
		}
