var search_process = function(){
  ...
  dnm_data.key = $("#key").val();

	var postdata ={
		scan_input:         dnm_data.scan_input,
		unique_key:			dnm_data.key,
	};
	...
	$.ajax({
		...
		$.ajax({
			type:'POST',
			url: 'plugin.php?page=Serials/model/json/search.php',
			data: postdata,
...
$.ajax({
    ...
    if (data.indexOf('ERROR')>-1){
        ...
        $("#virhe") .removeClass("alert-danger")
    		...
    		dnm_data.quantity_count += 1;
			console.log(dnm_data.quantity_count);
        ...
        if (dnm_data.quantity_count % 3 === 0)
            data_output  = "<div class='pull-left' style='padding: 0px 25px'>" + data_output + "</div><div class='clearfix'></div>";
        else data_output = "<div class='pull-left' style='padding: 0px 25px'>" + data_output + "</div>";
