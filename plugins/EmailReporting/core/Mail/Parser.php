...
class ERP_Mail_Parser
{
	...
	* http://lists.w3.org/Archives/Public/ietf-charsets/2001AprJun/0030.html */
	...
	private function parseStructure( &$structure )
	{
		...
		// using thread-index to determine if a email chain is a reply to an original
		if ( isset ( $structure->headers[ 'thread-index' ] ) )
		{
			$this->setThreadIndex( $structure->headers[ 'thread-index' ] );
		}	
	...
	// using thread-index to reference existing bug 
 	private function setThreadIndex( $p_threadindex )
	{
	...
	private function setBody( $body, $ctype_primary, $ctype_secondary, $charset )
	{
		...
		elseif ( $this->_parse_html && 'text' === $this->_ctype['primary'] && 'html' === $this->_ctype['secondary'] )
		{
			...
			// comment out ->plaintext to have HTML directly saved into Database as description field.
