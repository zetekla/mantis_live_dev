...
//			access_ensure_project_level( config_get('report_bug_threshold' ) );
			...
			//array loop for custom_field_index value generation, flawed in the assumption that the parse array would be order in the same as custom_field_id and custom_field_id are sequential.
/* 			$parse_arr = array("/\[Assembly Number:(.*?)\]/","/\[Revision:(.*?)\]/","/\[Work Order Number:(.*?)\]/");
			$c = 0;
			foreach ($parse_arr as $rgxstr){
				$c++;
				preg_match ($rgxstr, $str ,${'custom_field_'.$c});
				${'custom_field_'.$c}= ${'custom_field_'.$c}[1];
				}
			 */
			...
			foreach( $t_related_custom_field_ids as $t_id )
			{
				...
				// build search regex using custom_field name value.  this requires email template to mirror custom field name values.
				$search = "/\[" . lang_get_defaulted( custom_field_get_field( $t_id, 'name' ) ) . ":(.*?)\]/";
				preg_match ($search, $t_description ,${'custom_field_'.$t_id});
				${'custom_field_'.$t_id}= ${'custom_field_'.$t_id}[1];
...
/* 				if ( !gpc_isset_custom_field( $t_id, $t_def['type'] ) &&
				...
				if ( !custom_field_validate( $t_id, ${'custom_field_'.$t_id} ) )
				{
					error_parameters( lang_get_defaulted( custom_field_get_field( $t_id, 'name' ) ) );
			...
			foreach( $t_related_custom_field_ids as $t_id )
			{
				...
				# Do not set custom field value if parse returns no or blank result.
				if (${'custom_field_'.$t_id} == "" )
				{
					continue;
				}
				$t_def = custom_field_get_definition( $t_id );
				if( !custom_field_set_value( $t_id, $t_bug_id, ${'custom_field_'.$t_id}, false ) )
				{
					error_parameters( lang_get_defaulted( custom_field_get_field( $t_id, 'name' ) ) );
					trigger_error( ERROR_CUSTOM_FIELD_INVALID_VALUE, ERROR );
				}
		...
		$this->add_msg_id( $t_bug_id, $p_email[ 'Message-ID' ] , $p_email[ 'Thread-Index']);
	...
	private function add_file( $p_bug_id, &$p_part )
	{
		...
		else
		{
			...
			// added $t_opt_name . $t_part_name	as option to mci_file_add function resulting in file name being added to file title.
...
//				if ( !( $this->_mailbox[ 'mailbox_type' ] === 'IMAP' && ( $this->_mailbox[ 'port' ] <= 0 || $this->_mailbox[ 'port' ] === $this->_default_ports[ $this->_mailbox[ 'mailbox_type' ] ][ $t_def_mailbox_port_index ] ) ) )
	...
	private function add_msg_id( $p_bug_id, $p_msg_id , $p_thread_index)
	{
		if( $this->_mail_use_message_id )
		{
			if ( !is_blank( $p_msg_id ) )
			{
				...
				if( $t_bug_id === FALSE )
				{
					...
					$query = 'INSERT
						...
						( issue_id, msg_id, thread_index)
						VALUES
						( ' . db_param() . ', ' . db_param() . ', ' . db_param() .')';
					db_query_bound( $query, array( $p_bug_id, $p_msg_id, $p_thread_index ) );
				}
			}
		}
	}
