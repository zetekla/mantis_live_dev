...
class EmailReportingPlugin extends MantisPlugin
{
	...
	function install()
	{
		...
		if ( $t_mail_reporter_id === 'Mail' )
		{
			...
			# create the user
			$t_result_user_create = user_create( $t_realname, $t_username, $t_password, $t_email, config_get_global( 'report_bug_threshold' ), FALSE, TRUE, 'Mail Reporter', plugin_lang_get( 'plugin_title' ) );
